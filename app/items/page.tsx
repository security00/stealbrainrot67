                ...
                {rows.map((it) => {
                  ...
                  return (
                    <tr key={it.id} className="hover:bg-black/30">
                      ...
                      <td className="p-2 border-b border-cyan-500/20">{it.incomeText || (it.incomePerSecond ? it.incomePerSecond.toLocaleString() : '-') }</td>
                      <td className="p-2 border-b border-cyan-500/20">{it.dropRateText || (it.dropRate ? `${(it.dropRate*100).toFixed(2)}%` : '-')}</td>
